...
def run_notch(data, window=0.5, mindbic=-1.0, useraw=False):
    ...
    print('Running notch filter pipeline with windowsize of ' + str(window) +
          ' and minimum DeltaBIC of ' +str(mindbic))
...
def run_locor(data, prot, alias_num=0.01, useraw=False):
    ...
    print('Running LOCoR with rotation period of ' + str(prot) +
          ' and alias_num of ' + str(alias_num))

    fittimes, depth, detrend, polyshape, badflag = core.do_detrend(
        1, 1001, arclength=False, raw=useraw, wsize=prot, indata=data,
        saveoutput=False, outdir='', resolvabletrans=False, demode=2,
        alias_num=alias_num, cleanup=True
    )
...
def run_bls(data, targetlc, badflag, rmsclip=1.5, snrcut=7.0, searchmax=20.0,
            searchmin=1.00001, binn=300, mindcyc=0.005, maxdcyc=0.3,
            freqmode='standard'):
    '''
    A wrapper that runs the iterative BLS search via core.bls_transit_search
    parameters are set for generally use
    '''
    best_px, dpx, t0x, detsigx, firstpower, pgrid, firstphase, dcycx = (
        core.bls_transit_search(
            data, targetlc, badflag, rmsclip=rmsclip, snrcut=snrcut,
            searchmax=searchmax, searchmin=searchmin, binn=binn,
            period_matching=[-1, -1, -1], mindcyc=mindcyc, maxdcyc=maxdcyc,
            freqmode=freqmode, datamode='standard')
    )
    ##the change to t0 here makes transits at phase 0
    return (
        best_px, dpx, t0x+best_px/2.0, detsigx, firstpower, pgrid, firstphase,
        dcycx
    )


def run_bls_bic(
    data, targetbic, badflag, rmsclip=1.5, snrcut=7.0, searchmax=20.0,
    searchmin=1.00001, binn=300, mindcyc=0.003, maxdcyc=0.01,
    freqmode='standard'):
    '''
    A wrapper that runs the iterative BLS search on the BIC statistic via
    core.bls_transit_search parameters set for BIC specifically
    '''

    best_px, dpx, t0x, detsigx, firstpower, pgrid, firstphase, dcycx = (
        core.bls_transit_search(data, targetbic, badflag, rmsclip=rmsclip,
                                snrcut=snrcut, searchmax=searchmax,
                                searchmin=searchmin, binn=binn,
                                period_matching=[-1, -1, -1], mindcyc=mindcyc,
                                maxdcyc=maxdcyc, freqmode=freqmode,
                                datamode='bic')
    )

    return (
        best_px, dpx, t0x+best_px/2.0, detsigx, firstpower, pgrid,
        firstphase, dcycx
    )
